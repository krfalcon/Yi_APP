...
-(void)setImageUrl:(NSURL *)imageUrl
{
    ...
    //reset current image
    self.image = nil;
    self.imageDownloadUrl = [imageUrl absoluteString];
    self.userInteractionEnabled = NO;
    self.clipsToBounds = YES;
    self.contentMode = UIViewContentModeScaleAspectFill;
    
    NSData *imageLocalData = [ImageCoreDataHelper getOneImageByUrl:_imageDownloadUrl];
    
    if (imageLocalData) {
        self.image = [UIImage imageWithData:imageLocalData];
        if (self.isBlack) {
            self.image = [ImageUtil imageWithImage:self.image withColor:1];
        }
        return;
    }
...
//            _loadingIndicator.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.2];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    [ImageCoreDataHelper addImage:imageData WithUrl:_imageDownloadUrl];
